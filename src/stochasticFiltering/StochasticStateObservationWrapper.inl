...
void StochasticStateObservationWrapper<DataTypes, FilterType>::setSofaVectorFromObservationsStateVector(EVectorX& _observationsState, typename DataTypes::VecCoord& _vec) {
    ...
    for (helper::vector<std::pair<size_t, size_t> >::iterator it = observationPosPairs.begin(); it != observationPosPairs.end(); it++) {
        ...
        if ((Dim*it->second + Dim) > _observationsState.rows()) {
            PRNE("Accessing DA vector out of bounds: " << Dim*it->second + Dim <<  " vs. " << _observationsState.rows());
            return;
        }

        for (size_t d = 0; d < Dim; d++) {
            _vec[it->first][d] = _observationsState(Dim*it->second + d);
        }
        //PRNS("Setting free[" << it->first << "] = " << _vec[it->first]);
    }
}
...
template <class DataTypes, class FilterType>
void StochasticStateObservationWrapper<DataTypes, FilterType>::transformState(EVectorX &_vecX, const core::MechanicalParams *_mparams, int* _stateID) {
    EVectorX savedState;
...
template <class DataTypes, class FilterType>
void StochasticStateObservationWrapper<DataTypes, FilterType>::computeCollision(const core::ExecParams* params)
{
    {
...
template <class DataTypes, class FilterType>
void StochasticStateObservationWrapper<DataTypes, FilterType>::integrate(const core::ExecParams* params, SReal dt)
{
