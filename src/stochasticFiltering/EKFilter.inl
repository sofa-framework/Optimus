...
void EKFilter<FilterType>::blasMultAdd(EMatrixX& _a, EMatrixX& _b, EMatrixX& _c, Type _alpha, Type _beta) {
    char trans = 'N';
    int m = _a.rows();
    int n = _b.cols();
    int k = _b.rows();
...
template <class FilterType>
void EKFilter<FilterType>::computePrediction()
{
...
template <class FilterType>
void EKFilter<FilterType>::bwdInit() {
    PRNS("bwdInit");
...
template <class FilterType>
void EKFilter<FilterType>::initializeStep(const core::ExecParams* _params, const size_t _step) {
