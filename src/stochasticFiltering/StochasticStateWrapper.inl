...
template<>
class InternalCopy<sofa::defaulttype::Rigid3dTypes>
{
public:
    typedef typename sofa::defaulttype::Rigid3dTypes::VecCoord VecCoord;
    typedef typename sofa::defaulttype::Rigid3dTypes::Real Real;
    type::vector<type::Quat<Real>> m_ori;

    void copyStateToFilter(type::vector<std::pair<size_t, size_t> > & positionPairs, const VecCoord & pos)
    {
        m_ori.clear();

        for (type::vector<std::pair<size_t, size_t> >::iterator it = positionPairs.begin(); it != positionPairs.end(); it++)
        {
    ...
    void copyFilterToSofa(type::vector<std::pair<size_t, size_t> >& positionPairs, VecCoord& pos)
    {
        for (type::vector<std::pair<size_t, size_t> >::iterator it = positionPairs.begin(); it != positionPairs.end(); it++)
        {
            // for (unsigned i = 0; i < pos.size(); i++)
...
StochasticStateWrapper<DataTypes, FilterType>::StochasticStateWrapper()
    ...
    , posModelStdev( initData(&posModelStdev, type::vector<FilterType>(0.0), "posModelStdev", "standard deviation in observations") )
    , velModelStdev( initData(&velModelStdev, type::vector<FilterType>(0.0), "velModelStdev", "standard deviation in observations") )
    , paramModelStdev( initData(&paramModelStdev, type::vector<FilterType>(0.0), "paramModelStdev", "standard deviation in observations") )
    , d_positionStdev( initData(&d_positionStdev, type::vector<FilterType>(1, 0.0), "positionStdev", "estimate standard deviation for positions"))
    , d_velocityStdev( initData(&d_velocityStdev, type::vector<FilterType>(1, 0.0), "velocityStdev", "estimate standard deviation for velocities"))
...
template <>
void StochasticStateWrapper<sofa::defaulttype::Rigid3dTypes, double>::stateDim()
{
...
template <>
void StochasticStateWrapper<sofa::defaulttype::Vec3dTypes, double>::stateDim()
{
    posDim = 3;
    velDim = 3;
}
...
void StochasticStateWrapper<DataTypes, FilterType>::init()
{
    ...
    if ( this->d_langrangeMultipliers.getValue() ) {
...

        {
            ...
            core::behavior::MultiVecDeriv dx(&vop, core::VecDerivId::dx() );
            dx.realloc( &vop, true, true );
            core::behavior::MultiVecDeriv df(&vop, core::VecDerivId::dforce() );
            df.realloc( &vop, true, true );
        }
...
void StochasticStateWrapper<DataTypes, FilterType>::bwdInit()
{
    ...
    if ( fixedConstraint ) {
        ...
        for (size_t i = 0; i < mechanicalState->getSize(); i++) {
            type::vector<size_t>::iterator it = find(fixedNodes.begin(), fixedNodes.end(), i);
...
/**
 * function called from observation manager when it needs to perform mapping
 * (a more elegant solution could/should be found)
 */
template <class DataTypes, class FilterType>
...
void StochasticStateWrapper<DataTypes, FilterType>::setSofaVelocityFromFilterVector(EVectorX& _state, typename DataTypes::VecDeriv& _vel)
{
    ...
    for (type::vector<std::pair<size_t, size_t> >::iterator it = velocityPairs.begin(); it != velocityPairs.end(); it++) {
 ...
 */
template <>
void StochasticStateWrapper<sofa::defaulttype::Rigid3dTypes, double>::copyStateFilter2Sofa(const core::MechanicalParams* _mechParams, bool /* _setVelocityFromPosition */)
{
    ...
    if ( estimatePosition.getValue() && !estimateOnlyXYZ.getValue() ) {
        type::vector<type::Vector6> EulerPos;
        EulerPos.resize(pos.size());
        type::Quat<sofa::defaulttype::Rigid3dTypes::Real> q_ori;
        q_ori.normalize();
        type::Vector3 euler_ori;
        type::Vector3 eu_pos;
        for (type::vector< std::pair<size_t, size_t> >::iterator it = positionPairs.begin(); it != positionPairs.end(); it++)
        {
            for (size_t d = 0; d < posDim; d++) {
        ...
        for (size_t i = 0; i < pos.size(); i++)
        {
            ...
            q_ori = type::Quat<sofa::defaulttype::Rigid3dTypes::Real>::createQuaterFromEuler(euler_ori * M_PI / 180);
            q_ori.normalize();
    ...
    if (estimatePosition.getValue() && estimateOnlyXYZ.getValue()) {
        ...
        type::vector<type::Vector3> EulerPos;
        EulerPos.resize(pos.size());
        type::Vector3 eu_pos;
        ...
        {
            type::Vector3 P0(EulerPos[0][0], EulerPos[0][1], EulerPos[0][2]);
            type::Vector3 P1(EulerPos[1][0], EulerPos[1][1], EulerPos[1][2]);

            type::Vector3 X0 = P1 - P0;
            X0.normalize();
            type::Vector3 Y0, Z0;

            if (fabs(dot(X0, type::Vector3(1, 0, 0))) >= 0.999999999999999) {
                Y0 = cross(X0, type::Vector3(0, 1, 0));
            } else {
                Y0 = cross(X0, type::Vector3(1, 0, 0));
            }
            Y0.normalize();
            Z0 = cross(X0, Y0);
            Z0.normalize();
        ...
        {
            type::Vector3 P0(EulerPos[i][0], EulerPos[i][1], EulerPos[i][2]);
            type::Vector3 P1(EulerPos[i + 1][0], EulerPos[i + 1][1], EulerPos[i + 1][2]);

            type::Vector3 X = P1 - P0;
            X.normalize();
            type::Vector3 Yprec = pos[i - 1].getOrientation().rotate(type::Vector3(0, 1, 0));
            type::Vector3 Z = cross(X, Yprec);
            Z.normalize();

            type::Vector3 Y = cross(Z, X);
            Y.normalize();
        ...
        int end = pos.size() - 1;
        {
            type::Vector3 P0(EulerPos[end - 1][0], EulerPos[end - 1][1], EulerPos[end - 1][2]);
            type::Vector3 P1(EulerPos[end][0], EulerPos[end][1], EulerPos[end][2]);

            type::Vector3 X = P1 - P0;
            X.normalize();
            type::Vector3 Yprec = pos[end - 1].getOrientation().rotate(type::Vector3(0, 1, 0));
    ...
    if ( !estimateVelocity.getValue() )
    {
        if (velocityPairs.empty() && beginTimeStepPos.size() == pos.size())
        {
            ...
            type::vector<type::Vector6> begEulerPos;
            begEulerPos.resize(pos.size());
            type::Quat<sofa::defaulttype::Rigid3dTypes::Real> begq_ori;
            begq_ori.normalize();
            type::Vector3 begeuler_ori;
            type::Vector3 begeu_pos;
    ...
    sofa::simulation::mechanicalvisitor::MechanicalPropagateOnlyPositionAndVelocityVisitor(_mechParams).execute( this->gnode );
 ...
 */
template <>
void StochasticStateWrapper<sofa::defaulttype::Rigid3dTypes, double>::copyStateSofa2Filter()
{
    typename MechanicalState::ReadVecCoord pos = mechanicalState->readPositions();
    typename MechanicalState::ReadVecDeriv vel = mechanicalState->readVelocities();
...
{
    if (! (this->filterKind == CLASSIC || this->filterKind == REDORD || this->filterKind == ENSEMBLTRANSF) )
        return;
...
template <class DataTypes, class FilterType>
void StochasticStateWrapper<DataTypes, FilterType>::getActualMappedPosition(int _id, sofa::defaulttype::Vec3dTypes::VecCoord& _mapPos)
{
    if ( mappedState != NULL)
...
typename StochasticStateWrapper<DataTypes, FilterType>::EMatrixX& StochasticStateWrapper<DataTypes, FilterType>::getModelErrorVariance()
{
    ...
    {
        type::vector<FilterType> velModStDev, posModStDev;
        ...
        type::vector<FilterType> paramModelStDev = paramModelStdev.getValue();
        ...
        if (estimatePosition.getValue() && estimateVelocity.getValue()) {
            ...
            for (size_t pi = this->reducedStateIndex; pi < this->stateSize; pi++, k++) {
                if (paramModelStdev.isSet() == 0) {
                    ...
                    /// Non-null Q for Force
                    this->modelElementNoise(pi) = paramModelStDev[k] * paramModelStDev[k];
                }
            }
        }
...
void StochasticStateWrapper<DataTypes, FilterType>::transformState(EVectorX &_vecX, const core::MechanicalParams *_mparams, int* _stateID)
{
    ...
    if (this->filterKind == CLASSIC || this->filterKind == ENSEMBLTRANSF) {
        /// store the result of the simulation as a vector
        VecCoord actualPos(this->mStateSize);
        VecDeriv actualVel(this->mStateSize);
        ...
        if ( mappedState != NULL) {
            sofa::defaulttype::Vec3dTypes::VecCoord actualMappedPos(this->mappedMStateSize);
            typename MappedMechanicalState::ReadVecCoord mapPos = mappedState->readPositions();
...
void StochasticStateWrapper<DataTypes, FilterType>::computeSofaStepWithLM(const core::ExecParams* params)
{
    ...
    sofa::simulation::mechanicalvisitor::MechanicalVInitVisitor< core::V_COORD >(params, core::VecCoordId::freePosition(), core::ConstVecCoordId::position(), true).execute(this->gnode);
    sofa::simulation::mechanicalvisitor::MechanicalVInitVisitor< core::V_DERIV >(params, core::VecDerivId::freeVelocity(), core::ConstVecDerivId::velocity(), true).execute(this->gnode);
    ...
    sofa::simulation::mechanicalvisitor::MechanicalBeginIntegrationVisitor beginVisitor(params, dt);
    ...
    {
        simulation::mechanicalvisitor::MechanicalVOpVisitor lambdaMultInvDt(params, cparams.lambda(), sofa::core::ConstMultiVecId::null(), cparams.lambda(), 1.0 / dt);
        lambdaMultInvDt.setMapped(true);
        this->getContext()->executeVisitor(&lambdaMultInvDt);
        simulation::mechanicalvisitor::MechanicalComputeGeometricStiffness geometricStiffnessVisitor(&mop.mparams, cparams.lambda());
    ...
    if (cparams.constOrder() == core::ConstraintParams::POS || cparams.constOrder() == core::ConstraintParams::POS_AND_VEL)
    {
        // xfree = x + vfree*dt
        simulation::mechanicalvisitor::MechanicalVOpVisitor freePosEqPosPlusFreeVelDt(params, freePos, pos, freeVel, dt);
    ...
    simulation::mechanicalvisitor::MechanicalEndIntegrationVisitor endVisitor(params, dt);
...
void StochasticStateWrapper<DataTypes, FilterType>::integrate(const core::ExecParams* params, SReal dt)
{
    ...
    simulation::mechanicalvisitor::MechanicalIntegrationVisitor act( params, dt );
    act.setTags( this->getTags() );
    act.execute( this->getContext() );
