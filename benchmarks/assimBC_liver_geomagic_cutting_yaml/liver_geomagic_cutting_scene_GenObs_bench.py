...
class liver_geomagicControlPoint_GenObs (Sofa.PythonScriptController):
    ...
    def createGraph(self, rootNode):
        ...
        dotNode.createObject('SphereCollisionModel', template='Rigid3d', color='0.5 0.5 0.5 1', radius='0.014')
        if self.options['obs_generating_parameters']['save_observations']:
            ...
            dotNode.createObject('OptimMonitor', template='Rigid3d', name='toolMonitor', showPositions='1', indices='@geomagicBounds.indices', ExportPositions='1', fileName='observations/geomagic')
        ...
        if 'boundary_conditions_list' in self.options['general_parameters'].keys():
            for index in range(0, len(self.options['general_parameters']['boundary_conditions_list'])):
                ...
                if bcElement['condition_type'] == 'fixed':
                    ...
                    self.restSpring = simuNode.createObject('RestShapeSpringsForceField', listening='1', stiffness=bcElement['spring_stiffness_values'], printLog="0", points='@boundBoxes'+str(index)+'.indices')
                else:
