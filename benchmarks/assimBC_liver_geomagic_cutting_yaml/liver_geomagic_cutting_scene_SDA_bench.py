...
def createScene(rootNode):
    ...
    rootNode.addObject(LiverGeomagicControlPointSDA_Controller(name="LiverGeomagicControlPoint_SDA", node=rootNode, opt=options))
    return 0




class LiverGeomagicControlPointSDA_Controller(Sofa.Core.Controller):
    ...
    def createCommonComponents(self, node):
        ...
        node.addObject('BoxROI', name='impactBounds', box='0.14 0.15 0.37 0.18 0.17 0.4', doUpdate='0')
        ...
        if 'boundary_conditions_list' in self.options['general_parameters'].keys():
            for index in range(0, len(self.options['general_parameters']['boundary_conditions_list'])):
                ...
                if bcElement['condition_type'] == 'fixed':
                    ...
                    node.addObject('RestShapeSpringsForceField', stiffness='@paramE.value', listening="1", printLog="0", points='@boundBoxes'+str(index)+'.indices')
                else:
    ...
    def createMasterScene(self, node):
        ...
        obsNode.addObject('ShowSpheres', name="estimated", radius="0.003", color="1 0 0 1", position='@SourceMO.position')
