...
class liver_controlPoint_SDA(Sofa.PythonScriptController):
    ...
    def createGlobalComponents(self, rootNode):
        ...
        impactSimu.createObject('ShowSpheres', position='@state.position', color='1.0 0.0 1.0 1', radius="0.01", showIndicesScale='0.0')
            ...
            for index in range(0, len(self.options['boundary_parameters']['boundary_conditions_list'])):
                ...
                if bcElement['condition_type'] == 'fixed':
                    ...
                    if self.options['boundary_parameters']['spring_type'] == 'PolynomialRestshape':
                        ...
                        node.createObject('PolynomialSpringsForceField', polynomialStiffness='@paramE.value', howIndicesScale='0', springThickness="3", listening="1", updateStiffness="1", printLog="0", firstObjectPoints='@boundBoxes'+str(index)+'.indices', polynomialDegree=bcElement['polynomial_degrees'], external_rest_shape='../fixNode/fixElements', secondObjectPoints=self.options['boundary_parameters']['external_indices'])
                    else:
