...
def createScene(rootNode):
    ...
    rootNode.addObject(Cylinder3PartsRestShapeSDA_Controller(name="Cylinder3PartsRestShape_SDA", node=rootNode, opt=options))
    return 0




class Cylinder3PartsRestShapeSDA_Controller(Sofa.Core.Controller):
    ...
    def createCommonComponents(self, node):
        ...
        node.addObject('Indices2ValuesMapper', name='youngMapper', indices='1 3', values='@paramE.value', inputValues='@/loader.dataset', defaultValue='6000')
        ...
        node.addObject('BoxROI', name='impactBounds', box=self.options['impact_parameters']['bound'], doUpdate='0')
        self.toolSprings = node.addObject('RestShapeSpringsForceField', name="impactSpring", stiffness="10000", angularStiffness='1', external_rest_shape='@/externalImpSimu/state', drawSpring='1', points='@impactBounds.indices')
