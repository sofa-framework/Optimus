...
namespace Ops
{
  ...
  */
                  ...
                  std::vector<Tout>& out)
  {
    ...
    //IP if (lua_pcall(state_, int(in.size()), LUA_MULTRET, 0) != 0)
    //IP  throw Error("Apply(string, vector, vector&)",
    //IP              "While calling " + Function(name) + ":\n  "
    //IP              + lua_tostring(state_, -1));
  ...
  */
  ...
  {
    //IP if (luaL_dofile(state_, file_path.c_str()))
  ...
  */
  ...
  {
    //IP if (luaL_dostring(state_, expression.c_str()))
  ...
  */
  bool Ops::Convert(int index, string& output, string name)
  {
    ...
    //IP output = static_cast<string>(lua_tostring(state_, index));
    return true;
  }
