...
namespace Verdandi
{
    ...
    */
    ...
    ::Forward()
    {
	 ...
	 //std::cout << "U_inv = " << U_inv_(0,0) << " " << U_inv_(1,1) << " " << U_inv_(0,1) << " " << U_inv_(1,0)  << std::endl;
            GetCholesky(U_inv_);
            Copy(model_.GetStateErrorVarianceProjector(), tmp);
            MltAdd(Ts(1), tmp, U_inv_, Ts(0),
                   model_.GetStateErrorVarianceProjector());
		...
		//std::cout << "L = \n" << tmp << std::endl;
                ...
                model_state_error_variance tmp2(Nstate_, Nreduced_), tmp3(Nstate_, Nstate_);
		tmp2.Zero();
		tmp3.Zero();
                MltAdd(Ts(1), tmp, U_inv_, Ts(0), tmp2);
                MltAdd(Ts(1), SeldonNoTrans, tmp2, SeldonTrans, tmp, Ts(0), tmp3);
		char fname[100];
                sprintf(fname, "outmat/Pf_%04d.txt", nstep);
	    ...
	    model_state_error_variance pertMat(X_i_trans_.GetM(), X_i_trans_.GetN()); 
	    pertMat.Zero();
            ...
            MltAdd(Ts(1), SeldonNoTrans, I_trans_, SeldonTrans,
                ...
                sprintf(fname, "outmat/Pert_%04d.txt", nstep);
                ofs.open(fname);
                ofs << pertMat << std::endl;
                ofs.close();
    ...
    ::Analyze()
    {
        ...
        else
        {
            ...
            if (alpha_constant_)
            {
                ...
                model_state_error_variance tmp, tmp2(Nstate_, Nreduced_), tmp3(Nstate_, Nstate_);
		...
		char fname[100];
		sprintf(fname, "outmat/P_%04d.txt", nstep);
		std::ofstream ofs(fname);
                ofs << tmp3 << std::endl;
		ofs.close();

		sprintf(fname, "outmat/UI_%04d.txt", nstep);
		ofs.open(fname);
                ofs << U_inv_ << std::endl;
		ofs.close();

		sprintf(fname, "outmat/L_%04d.txt", nstep);
		ofs.open(fname);
                ofs << tmp << std::endl;
		ofs.close();

		sprintf(fname, "outmat/X_%04d.txt", nstep);
		ofs.open(fname);
                ofs << model_.GetState() << std::endl;
		ofs.close();
	...
	model_state& xx = model_.GetState();
	size_t ll = xx.GetM(); 
	std::cout << "Parameters: " << xx(ll-2) << " " << xx(ll-1) << std::endl;
#ifndef VERDANDI_WITH_MPI
