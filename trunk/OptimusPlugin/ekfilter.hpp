...
namespace Kalman {
  ...
  template<typename T, bool OQ = false, bool OVR = false>
  class EKFilter {
  public:

    typedef T type;     //!< Type of objects contained in matrices and vectors.

    typedef sofa::helper::vector<T> Vector;  //!< Vector type.
    //typedef sofa::defaulttype::Mat<T> Matrix;  //!< Matrix type.
    typedef sofa::component::linearsolver::FullMatrix<T> Matrix;  //!< Matrix type.
    ...
    //! arguments.
    EKFilter(unsigned int n_, unsigned int nu_, unsigned int nw_, unsigned int m_, unsigned int nv_);
    ...
    void setDim(unsigned int n_, unsigned int nu_, unsigned int nw_, unsigned int m_, unsigned int nv_);
    ...
    //! \warning \c init() must always be called after this function and
    //! before any other non-dimensioning function.
    void setSizeX(unsigned int n_);

    //! Sets the size of the input vector.
    void setSizeU(unsigned int nu_);
    ...
    //! \warning \c init() must always be called after this function and
    //! before any other non-dimensioning function.
    void setSizeW(unsigned int nw_);

    //! Sets the size of the measurement vector.
    void setSizeZ(unsigned int m_);

    //! Sets the size of the measurement noise vector.
    void setSizeV(unsigned int nv_);
    ...
    //! function.
    void initFilter(Vector& x_, Matrix& P_);
  ...
  protected:
    ...
    unsigned int n;        //!< Size of the state vector.
    unsigned int nu;       //!< Size of the input vector.
    unsigned int nw;       //!< Size of the process noise vector.
    unsigned int  m;       //!< Size of the measurement vector.
    unsigned int nv;       //!< Size of the measurement noise vector.
  ...
  private:
    ...
    //! right part.
    unsigned int nn;       //!< Number of columns of \a U

    mutable Matrix _P;  //!< Temporary matrix.
    mutable Vector _x;  //!  Temporary vector.

    unsigned int flags;    //!< Bitfield keeping track of modified matrices.
