...
\def\State{\mathbf{s}}
\def\Params{\mathbf{p}}
\def\Control{\mathbf{c}}


\def\sstate{\textit{{S-state}}}
\def\mstate{\textit{{M-state}}}
\def\vstate{\textit{V-state}}
\def\svda{\textit{SVDA}}


\def\smp{$\sigma-$\textit{point}}
\def\smw{\texttt{SofaModelWrapper}}
\def\roukf{\texttt{ReducedOrderUKF}}
...
\section{Data Assimilation Using Verdandi}

{\large\bf This section is now obsolete, since Verdandi was replaced with our own implementation of filters. Nonetheless, in the new version, the concepts remain very similar: mainly, the Verdandi-related stuff has been replaced by SOFA-filtering (as explained above). Results are still relevant showing the behaviour of both the previous and actual code.}
