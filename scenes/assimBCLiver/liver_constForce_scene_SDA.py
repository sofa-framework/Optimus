...
def createScene(rootNode):
    ...
    rootNode.addObject(liverConstForceSDA_Controller(name="liverConstForce_SDA", node=rootNode, opt=options))
    return 0



# Class definition
class liverConstForceSDA_Controller(Sofa.Core.Controller):
    ...
    def createCommonComponents(self, node):
        ...
        node.addObject('TetrahedronFEMForceField', name='FEM', updateStiffness='1', listening='true', drawHeterogeneousTetra='1', method='large', youngModulus='5000', poissonRatio='0.45')

        # boundary conditions and their estimated parameters
        ...
        if 'boundary_conditions_list' in self.options['general_parameters'].keys():
            for index in range(0, len(self.options['general_parameters']['boundary_conditions_list'])):
                ...
                if bcElement['condition_type'] == 'fixed':
                    ...
                    node.addObject('RestShapeSpringsForceField', stiffness='@paramE.value', drawSpring='1', listening="1", printLog="0", points='@boundBoxes'+str(index)+'.indices')
                else:
                    print('Unknown type of boundary conditions')

        # constant force field
        node.addObject('BoxROI', name='impactBounds', box='0.14 0.15 0.4 0.16 0.17 0.43', doUpdate='0')
        node.addObject('ConstantForceField', name='appliedForce', indices='@impactBounds.indices', totalForce='0.0 -2.0 0.9')
