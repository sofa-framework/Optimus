...
# Class definition 
class synth1_BCDA_PreStoch(Sofa.PythonScriptController):
    ...
    def createPreStochasticScene(self, node):
        node.createObject('PreStochasticWrapper')
        # node.createObject('VisualStyle', name='VisualStyle', displayFlags='hideForceFields')
        node.createObject('Gravity', gravity='0 -9.81 0')
        node.createObject('EulerImplicitSolver')
        node.createObject('BTDLinearSolver')        
        node.createObject('MechanicalObject', name='MO', template='Rigid', position='0.1 0 0 0 0 0 1  0.11 0 0  0 0 0 1  0.12 0 0  0 0 0 1  0.13 0 0 0 0 0 1 0.14 0 0  0 0 0 1  0.15 0 0 0 0 0 1')
        node.createObject('Mesh', name='BeamTopo', position='@MO.position', edges='0 1  1 2  2 3  3 4  4 5')
        node.createObject('UniformMass', totalmass='0.001', showAxisSizeFactor="0.003")
        node.createObject('BeamFEMForceField', name='BeamFEM', radius='0.003', youngModulus='5e4', poissonRatio='0.40')
        node.createObject('FixedConstraint', indices='0')        
        
        # node.createObject('Sphere', radius="0.01")    
        return 0
    ...
    def createScene(self,node):
        ...
        prestochNode=node.createChild('Prestochastic')
        self.createPreStochasticScene(prestochNode)
