...
class cylConstForce_SDA(Sofa.PythonScriptController):
    ...
    def createGraph(self, rootNode):
        ...
        self.period = self.options['impact_parameters']['force_period']
        self.amplitude = self.options['impact_parameters']['force_amplitude']
        self.iterations = 0
        ...
        if self.options['additional_parameters']['compute_bound_box']:
            self.globalMin = [10e8, 10e8, 10e8]
            self.globalMax = [-10e8, -10e8, -10e8]
    ...
    def createCommonComponents(self, node):                                  
        ...
        self.model = node.createObject('MechanicalObject', src="@/loader", name="Volume")
        ...
        self.oppositeConstantForce = node.createObject('ConstantForceField', name='oppAppliedForce', indices='@oppForceBounds.indices', totalForce='0.0 0.0 0.0')
    ...
    def createTimeProfiler(self):
        print 'Time statistics file: ' + self.fullFolderName + '/' + self.options['time_parameters']['computation_time_file_name']
        ...
        with open(self.fullFolderName + '/' + self.options['time_parameters']['computation_time_file_name'], "a") as outputFile:
            outputFile.write('{')
            outputFile.close()
    ...
    def saveBoundBox(self):
        if self.options['additional_parameters']['compute_bound_box']:
            ### compute local bound box
            localMin = [10e8, 10e8, 10e8]
            localMax = [-10e8, -10e8, -10e8]
            positions = self.model.findData('position').value
            for index in range(0, len(positions)):
                localMin[0] = min(localMin[0], positions[index][0])
                localMin[1] = min(localMin[1], positions[index][1])
                localMin[2] = min(localMin[2], positions[index][2])
                localMax[0] = max(localMax[0], positions[index][0])
                localMax[1] = max(localMax[1], positions[index][1])
                localMax[2] = max(localMax[2], positions[index][2])
            ### save data
            if self.iterations <= self.options['time_parameters']['iteration_amount']:
                with open(self.fullFolderName + '/' + self.options['additional_parameters']['bound_box_file'], "a") as outputFile:
                    outputFile.write("Min: " + ','.join(str(e) for e in localMin) + "; Max: " + ','.join(str(e) for e in localMax) + "\n")
                    outputFile.close()
                self.globalMin[0] = min(self.globalMin[0], localMin[0])
                self.globalMin[1] = min(self.globalMin[1], localMin[1])
                self.globalMin[2] = min(self.globalMin[2], localMin[2])
                self.globalMax[0] = max(self.globalMax[0], localMax[0])
                self.globalMax[1] = max(self.globalMax[1], localMax[1])
                self.globalMax[2] = max(self.globalMax[2], localMax[2])
            # replace last symbol
            if self.iterations == self.options['time_parameters']['iteration_amount']:
                with open(self.fullFolderName + '/' + self.options['additional_parameters']['bound_box_file'], "a") as outputFile:
                    outputFile.write("Global min: " + ','.join(str(e) for e in self.globalMin) + "; Global max: " + ','.join(str(e) for e in self.globalMax) + "\n")
                    outputFile.close()
    ...
    def exportStochasticState(self):
        ...
        if self.forceIndex == 0:
            self.constantForce.findData('totalForce').value = [0.0, self.amplitude * math.sin(math.pi * self.iterations / self.period), 0.0]
            ...
            self.oppositeConstantForce.findData('totalForce').value = [0.0, self.amplitude * math.sin(math.pi * self.iterations / self.period), 0.0]
