    ...
    def createGraph(self,rootNode):
        ...
        if self.filterKind == 'ROUKF':
            self.filter = rootNode.createObject('ROUKFilter', name="ROUKF", verbose="1", useBlasToMultiply='1', 
                sigmaTopology=self.opt['filter']['sigma_points_topology'], executeSimulationForCorrectedData=self.opt['filter']['ROUKFRecompSim'])
            estimatePosition = 1            
        ...
        if self.filterKind == 'UKFClassic':
            ...
            simuNode.createObject('StochasticStateWrapper',name="StateWrapper",verbose='1', printLog='1', 
                langrangeMultipliers=0, estimatePosition=estimatePosition, estimateVelocity='0', draw='1', radiusDraw='0.0002', )
