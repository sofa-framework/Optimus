...
class AppStiff_SDA(Sofa.PythonScriptController):
    ...
    def createGraph(self, rootNode):
        ...
        if self.filterKind == 'ROUKF':
            self.filter = rootNode.createObject('ROUKFilter', name="ROUKF", verbose="1", useBlasToMultiply='1', sigmaTopology=self.opt['filter']['sigma_points_topology'], executeSimulationForCorrectedData=self.opt['filter']['ROUKFRecompSim'])
            estimatePosition = 1
        ...
        if  method[0:3] == 'Cor':
            ...
            simuNode.createObject('TetrahedronHyperelasticityFEMForceField', name='FEM', materialName='StVenantKirchhoff', ParameterSet='@pm.outputValues')
