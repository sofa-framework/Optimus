...
class synth1_BCDA(Sofa.PythonScriptController):
    ...
    def createGlobalComponents(self, node):
        ...
        if (self.filterKind == 'ROUKF'):
            ...
            self.filter = node.createObject('UKFilterSimCorr', name="UKF", printLog='1', verbose='1', sigmaTopology="Simplex", useUnbiasedVariance="0",  lambdaScale="1")
    ...
    def createCommonComponents(self, node):
        ...
        if self.integration == 'Euler':
            ...
            node.createObject('StaticSolver', name="NewtonStatic", printLog="0", correction_tolerance_threshold="1e-8", residual_tolerance_threshold="1e-8", should_diverge_when_residual_is_growing="1", newton_iterations=self.numIter)
        elif self.integration == 'VarSym':
            node.createObject('VariationalSymplecticSolver', rayleighStiffness='1', rayleighMass='1', newtonError='1e-12', steps=self.numIter, verbose='0', useIncrementalPotentialEnergy='1')
        else:
        ...
        if self.estimQuantity == 'springs':
            self.optimParams = node.createObject('OptimParams', name="StochasticParams", template="Vector", numParams="@fixedBox1.nbIndices", initValue=self.paramInitExp, stdev=self.paramInitSD, transformParams=self.transformParams, optimize="1", printLog="1")
            node.createObject('RestShapeSpringsForceField', name="fixedSpring", points="@fixedBox1.indices", stiffness="@StochasticParams.value", showIndicesScale='0.05', listening="1", printLog="0")
            node.createObject('OglColorMap',colorScheme="Blue to Red")

        elif self.estimQuantity == 'forces':
            self.optimParams = node.createObject('OptimParams', name="StochasticParams", template="VecDeriv3d", numParams="@fixedBox1.nbIndices", initValue=self.paramInitExp, stdev=self.paramInitSD, transformParams=self.transformParams, optimize="1", printLog="1")
        ...
        # fixedA.createObject('SphereCollisionModel', color='1.0 0.0 0.0 1', radius="0.0019", template='Vec3d')

        # fixedB = node.createChild('fixedNB')
        # fixedB.createObject('MechanicalObject',name='MO', position='@../fixedB.indices_position')
        # fixedB.createObject('SphereCollisionModel', color='0.0 0.8 0.0 1', radius="0.0019", template='Vec3d')

        # fixedC = node.createChild('fixedNC')
        # fixedC.createObject('MechanicalObject',name='MO', position='@../fixedC.indices_position')
        # fixedC.createObject('SphereCollisionModel', color='1.0 0.64 0.0 1', radius="0.0019", template='Vec3d')

        # fixedD = node.createChild('fixedND')
        # fixedD.createObject('MechanicalObject',name='MO', position='@../fixedD.indices_position')
        # fixedD.createObject('SphereCollisionModel', color='0.0 1.0 1.0 1', radius="0.0019", template='Vec3d')
        ...
        node.createObject('VTKExporterDA', filename=self.outDir+'/DAobject.vtk', XMLformat='0', listening='1', edges="0", triangles="0", quads="0", tetras="1", exportAtBegin="1", exportAtEnd="0", exportEveryNumberOfSteps="1")
    ...
    def createMasterScene(self, node):
        ...
        # obsVisuNode.createObject('SphereCollisionModel', radius="0.002", color="0.2 0.8 0.2 1")
