...
class synth1_GenObs(Sofa.PythonScriptController):
    ...
    def createGraph(self, rootNode):
        ...
        if self.integration == 'Euler':
            ...
            simuNode.createObject('StaticSolver', name="NewtonStatic", printLog="1", correction_tolerance_threshold="1e-8", residual_tolerance_threshold="1e-8", should_diverge_when_residual_is_growing="1", newton_iterations=self.numIter)
        ...
        # simuNode.createObject('SphereCollisionModel', radius="0.0003")
        ...
        # simuNode.createObject('RestShapeSpringsForceField', name='fixingSpring', stiffness='1e5', points='@FROI1.indices', showIndicesScale='0', springColor='0 1 0 1')
        self.constForce=simuNode.createObject('ConstantForceField', indices='@FROI1.indices', forces=[0,0,0])

        # simuNode.createObject('RestShapeSpringsForceField', stiffness='1e5', name='fixingSpring', points='@FROI2.indices', springColor='0 1 0 1')
        ...
        mappedTool2.createObject('TriangleSetTopologyContainer', name='MappedContainer2', triangles="0 3 1 1 3 4 1 4 2 2 4 5 3 6 4 4 6 7 4 7 5 5 7 8")
        # self.toolSprings2=mappedTool.createObject('RestShapeSpringsForceField', name='toolSpring', stiffness='1e5', springColor='0 1 0 1', drawSpring='1', printLog='0', listening='1', external_rest_shape='@/tool/MO')
    ...
    def onBeginAnimationStep(self, deltaTime):
        self.step += 1
        if (self.step < 201):
            actForce=self.step/200.0*np.array([[-0.2578, -0.7552, 0.0275]])
            self.constForce.findData('forces').value = actForce.tolist() # [ [0.0,-0.1,0.0] ]
        return 0;
